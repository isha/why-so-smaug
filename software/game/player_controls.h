...
void read_switches() {
		...
		new_switch_values = IORD_32DIRECT(SWITCHES_BASE, 0);
...
bool move_right (Player * player) {
	return player->type == PLAYER1 ? player1_switches[0] : player2_switches[0];
}
bool move_down (Player * player) {
	return player->type == PLAYER1 ? player1_switches[1] : player2_switches[1];
}
bool move_up (Player * player) {
	return player->type == PLAYER1 ? player1_switches[2] : player2_switches[2];
}
bool move_left (Player * player) {
	return player->type == PLAYER1 ? player1_switches[3] : player2_switches[3];
}
